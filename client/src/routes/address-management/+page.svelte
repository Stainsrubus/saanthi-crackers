...
{#if isLoggedIn}
<div class="max-w-2xl md:mx-auto mx-4  p-4 border lg:my-20 md:my-16 my-10 rounded-lg shadow scrollbar-hide">
  <div class="flex justify-between flex-wrap items-center mb-4">
    <h2 class="lg:text-3xl md:text-2xl text-lg text-[#30363C] font-semibold">Address Management</h2>
    <Button onclick={openDialog} class="custom-button  text-white px-4 py-2 rounded-md hover:scale-105 transition-all duration-300">
      Add Address
    </Button>
  </div>
        ...
        <div class="border rounded-md p-4 bg-white shadow-sm">
          <div class="flex justify-between">
            <div class="space-y-2">
              ...
              <p class="font-medium md:text-lg text-sm text-[#4F585E]"> {address.addressType}</p>
              <p class="font-semibold md:text-xl text-base text-[#30363C] capitalize">{address.flatorHouseno}, {address.area}, {address.landmark}</p>
...
<div class="z-50">
  <h2 class="text-lg z-50 font-semibold mb-4 sticky top-0 flex items-center gap-2"><Icon onclick={closeDialog} icon="lucide:arrow-left" class="w-5 h-5 text-[#4F585E] cursor-pointer]"  /> {selectedAddress ? 'Edit Address' : 'Add Address'}</h2>
   ...
   <div class="overflow-y-auto h-[95%] scrollbar-hide">
    ...
    <div class="mt-6">
      <Button onclick={handleSaveAddress} disabled={$addAddressMutation.isPending || $updateAddressMutation.isPending} class="w-full text-base custom-button mb-10">
  ...
  <div class={`fixed inset-0 bg-black/50 flex justify-center items-center z-50 ${isDeleteConfirmOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
    <div class="bg-white p-6 rounded-lg shadow-lg lg:w-1/3 md:w-2/3 w-5/6">
...
<p class="md:text-lg text-base font-medium">Please login to Add Address</p>
