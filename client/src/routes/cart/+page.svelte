...
function resetFileInput() {
  ...
  function handleFileChange(event: Event) {
    ...
    if (input.files && input.files.length > 0) {
      ...
      for (const file of files) {
        ...
        //   toast.error('Each file should be less than 500KB');
        //   return;
        // }
      }
  ...
  function toggleCoupon() {
    ...
    console.log('Toggling coupon, cartItems:', cartItems);
    couponCode = '';
    $cartQuery.refetch();
    if (cartItems.length !== 0) {
  ...
  <div class="flex lg:flex-row flex-col lg:w-full md:w-[65%] w-full justify-between xl:px-12 lg:px-8 px-4 md:px-6 gap-5">
    <div class="border bg-white lg:hidden max-w-3xl flex justify-between rounded-lg shadow-lg p-3">
      ...
      <button
        ...
        class="h-fit custom-button text-white px-3 py-1.5 rounded-md cursor-pointer lg:text-lg text-base whitespace-nowrap font-medium"
      >
  ...
  +
    ...
    <div class="billing lg:w-[25%] md:max-w-2xl max-w-3xl">
      <div class="border hidden bg-white lg:flex justify-between rounded-lg shadow-lg p-3">
        ...
        <button
          onclick={handleAddressClick}
          class="h-fit custom-button text-white px-4 py-2 rounded-md cursor-pointer text-lg whitespace-nowrap font-medium"
        >
          {primaryAddress && !showShimmer ? 'Edit address' : 'Add address'}
        </button>
      </div>
      ...
      <div class="summary lg:mt-5 bg-white rounded-lg shadow-lg p-3 border">
        ...
        {:else}
            ...
            {#if isCouponVisible}
              <div class="relative w-full">
                <input
                  ...
                  class="border-primary w-full border text-sm rounded-md p-1 pl-2 pr-20 placeholder:text-xs focus:ring-0 focus:outline-none"
          ...
          <div class="relative">
            <div 
              class="custom-button cursor-pointer w-full mt-2 rounded-lg text-white p-1 font-bold text-center"
              ...
              <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                <div class="max-h-[90dvh] relative overflow-y-auto bg-white rounded-lg p-6 w-full max-w-md">
                  ...
                  <div class="pt-4 flex items-center justify-center flex-col gap-3">
                    ...
                    {/if}
                    <div>
                      <p class="text-primary">Supported Type: JPEG, PNG, PDF</p>
                    </div>
                    <Button 
...
<div class="container max-w-2xl my-20 py-20 rounded-lg shadow-lg flex-col gap-3 flex justify-center items-center">
  ...
  <button onclick={() => goto('/login')} class="custom-button hover:scale-105 rounded-lg px-4 text-lg text-white py-2">Login</button>
</div>
{/if}
