<script lang="ts">
	...
	let paymentStatusDialogOpen = $state(false);
	...
	function openPaymentStatusDialog(status: string) {
	  ...
	  paymentStatusDialogOpen = true;
	}
  
	async function updatePaymentStatus() {
	  try {
		await _axios.patch(`/orders/verify-payment/${$query.data?.order?._id}`, {
  ...
  <div class="w-[95%] mx-auto p-6 space-y-6 h-[calc(100vh-60px)] overflow-y-auto hidescrollbarthumb text-zinc-700">
	{#if $query.isLoading}
	...
	{:else if $query.data?.order}
	  ...
	  <div class="grid grid-cols-3 gap-4">
		<Card.Root class="mt-4 col-span-1 bg-zinc-100 border-primary border-2 border-solid">
		  <Card.Header>
		...
		<Card.Root class="mt-4 bg-zinc-100 border-primary border-2 border-solid">
		  <Card.Header>
		  ...
		  <Card.Content>
			<div class="grid grid-cols-2 gap-4">
			  ...
			  <div>
				...
				<Button
				  onclick={() => openPaymentStatusDialog(order.paymentStatus)}
				  class="w-fit px-2 capitalize"
			...
			{#if order.paymentImages && order.paymentImages.length > 0}
			  <div class="mt-4">
				<h3 class="font-bold text-[#30363C] mb-2">Payment Receipt</h3>
				  ...
				  {#each order.paymentImages as image}
					<div class="mb-2">
					  <img
						...
						alt="Payment Receipt"
						class="w-20 h-20 object-cover border p-2 rounded-lg cursor-pointer"
						onclick={() => openImageDialog(imgUrl + image.image)}
					  />
					</div>
				  {/each}
				</div>
			...
			<Table.Body>
			  {#each order.products as product, index}
				<Table.Row>
				  ...
				  <Table.Cell>
					<p class="font-medium capitalize">{product.productId?.productName || 'Unknown Product'}</p>
				  </Table.Cell>
				  <Table.Cell>{product.quantity}</Table.Cell>
				  <Table.Cell>₹{product.price.toFixed(2)}</Table.Cell>
				  <Table.Cell>₹{product.totalAmount.toFixed(2)}</Table.Cell>
				</Table.Row>
			  {/each}
			</Table.Body>
		  </Table.Root>
		...
		<Card.Content>
		  <div class="space-y-2">
			...
			<div class="flex justify-between font-bold text-lg pt-2 border-t">
			  <span>Total</span>
			  <span>
				₹{order?.totalPrice - (order?.couponDiscount > 0 ? order?.couponDiscount : 0)}
			  </span>
			</div>
		  </div>
		</Card.Content>
  ...
  <Dialog.Root bind:open={paymentStatusDialogOpen}>
	<Dialog.Content class="sm:max-w-[425px]">
	  ...
	  <Dialog.Description>
		<div class="space-y-4 mt-4">
		  <Button
			...
			onclick={() => { selectedPaymentStatus = 'initiated'; updatePaymentStatus(); }}
			disabled={selectedPaymentStatus === 'initiated'}
		  >
			Initiated
		  </Button>
		  <Button
			class="w-full capitalize bg-green-600 hover:bg-green-700"
			onclick={() => { selectedPaymentStatus = 'completed'; updatePaymentStatus(); }}
			disabled={selectedPaymentStatus === 'completed'}
		  >
			Completed
		  </Button>
		  <Button
			class="w-full capitalize bg-red-600 hover:bg-red-700"
			onclick={() => { selectedPaymentStatus = 'failed'; updatePaymentStatus(); }}
			disabled={selectedPaymentStatus === 'failed'}
		  >
			Failed
		  </Button>
		</div>
	  ...
	  <Dialog.Footer class="gap-2 mt-4">
		<Button
		  variant="outline"
		  onclick={() => paymentStatusDialogOpen = false}
		>
		  Cancel
		</Button>
	  </Dialog.Footer>
	...
	<Dialog.Content class="sm:max-w-[600px]">
	  ...
	  <Dialog.Description>
		<div class="flex justify-center">
		  <img src={selectedImage} alt="Payment Receipt" class="max-w-full max-h-[500px] object-contain" />
		</div>
	  </Dialog.Description>
	  <Dialog.Footer class="gap-2 mt-4">
		<Button
