...
.post(
  ...
  async ({ set, query, store }) => {
    ...
    try {
      ...
      const storeAddress = `${Estore.storeAddress}\n${Estore.storePhone}`;
      ...
      invoiceContent = invoiceContent
        ...
        // )
        .replace("{{store_name}}", Estore.storeName)
        .replace("{{store_phone}}", Estore.storePhone)
        .replace("{{store_email}}", Estore.storeEmail)
      ...
      browser = await puppeteer.launch({
        ...
        browser: "chrome",
        executablePath:
          "/var/www/src/api/chrome/linux-140.0.7296.0/chrome-linux64/chrome",
