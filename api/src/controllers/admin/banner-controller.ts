...
import { saveFile, deleteFile } from "@/lib/file-s3";
  ...
  .post(
    ...
    async ({ body }) => {
      try {
        ...
        if (currentBanners >= 5) {
          throw new Error("Limit reached. Only 5 banners are allowed.");
        }
    ...
    },
    {
      ...
      body: t.Object({
        ...
        bannerTitle: t.Optional(t.String({
        })),
        bannerDescription: t.Optional(t.String({
        })),
