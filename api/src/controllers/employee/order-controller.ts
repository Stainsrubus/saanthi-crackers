...
.get(
  ...
  async ({ params, store, set }) => {
    try {
      ...
      if (!empId) {
        set.status = 401;
        return { message: "Unauthorized: Employee ID missing", status: false };
      }

      // 1. Find the order
      let order;
      try {
        order = await EmpOrderModel.findOne({ _id: orderId, employee: empId });
      } catch (err) {
        console.error("Invalid orderId:", err);
        set.status = 400;
        return { message: "Invalid Order ID format", status: false };
      }
      ...
      try {
        broadcastMessage(
          `Order ID: ${order._id.toString()} was cancelled by ${emp.name}`
        );
      } catch (err) {
      ...
      await EmpOrderModel.findByIdAndUpdate(orderId, { status: "cancelled" });
